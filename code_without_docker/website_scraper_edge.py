...
import boto3
import glob
import edge_connection
import psycopg2
import psycopg2.extras as extras
import pandas as pd
from edge_connection import Migration
...
class scrapper(Migration):
    ...
    def click_on_view_all(self):
        locate_view_all = self.driver.find_element(
            By.XPATH, "//a[normalize-space()='View all']")
    ...
    def dump_raw_data_to_s3(self, raw_datas,product_unik):
        ...
        s3.put_object(Body=json.dumps(data), Bucket=bucket_name,
                      Key=f'website_data/{product_unik}')
    ...
    def uploadDirectory(self):
        ...
        try:
            s3 = boto3.client('s3')
            bucket_name = 'aicorbuc'
            files_in_image_folder = glob.glob(
                'C:\\Users\\Maud\\Desktop\\python\\data_pipelines\\data_pipeline\\Scripts\\raw_data\\images\\*')

            for file in files_in_image_folder:
                file_name = file.split('-')[-1]
                s3.upload_file(file, bucket_name,
                               f'website_data/images/{file_name}')
        except:
                ...
                for each_a_tag in a_tags:
                    ...
                    try:
                        urllib.request.urlretrieve(
                            prd_img, f'C:\\Users\\Maud\\Desktop\\python\\data_pipelines\\data_pipeline\\Scripts\\raw_data\\images\\{img_name}.jpg')
                    except:
                        urllib.request.urlretrieve(
                            prd_img, f'C:\\Users\\Maud\\Desktop\\python\\data_pipelines\\data_pipeline\\Scripts\\raw_data\\images\\{img_name}.png')
